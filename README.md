# Flask for full stack web development

- [Initial setup](docs/flask/initial_setup.md)
- [Running and configuring dev server](docs/flask/dev.md)
- [Using html and css templates](docs/flask/templates.md)
- [Request and response](docs/flask/request-response.md)
- [Using GET method](docs/flask/get.md)
- [Using POST method](docs/flask/post.md)
- [Sending JSON response](docs/flask/json.md)
- [Database - Pymongo](docs/flask/database.md)
- [Web forms and Flask Security](docs/flask/forms-security.md)
- [Flask REST APIs](docs/flask/rest-api.md)
